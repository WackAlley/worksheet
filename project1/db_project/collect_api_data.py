...
#A simplified version is available in the project folder (Jupyter Notebook). It is recomanded to review and understand that first first.
...
def request_data(start_time, products = {}):
    ...
    if products: # not true if products is empty
        request_specifications['products'] = products
...
while True:
    ...
    updated_data = request_data(start_time_today, products)
    ...
    for _ in range(int(seconds_until_midnight() / 60 / intervall_minitues)-1): # one safety intervall period to be extra sure to finish before midnight in addition int(...) rounds off
        ...
        new_data = request_data(start_time_today, products)
        #save request (backup):
