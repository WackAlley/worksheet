...
class Game():
    ...
    def __init__(self,
        ...
        self.spawn_goal_field = spawn_goal_field
        if spawn_goal_field:
            self.goal = Goal(np.array([size-1, 0], dtype=np.int8))
        if not rng:
    ...
    def _generate_board(self) -> NDArray[np.int8]:
        ...
        if self.spawn_goal_field:
            board[tuple(self.goal.pos)] = Obstacle.GOAL.value
    ...
    def act(self, action: Optional[Action]) -> bool:
        """
        Returns if goal was hit
        """
        if action is None:
        ...
        if action.is_move():
            ...
            # Reached goal
            if met_entity_encoded == Obstacle.GOAL.value:
                # other player loses
                del self.players[(self.next_player + 1) % self.n_players]
                self.next_player = (self.next_player + 1) % self.n_players
                self._board_cache = None
                return True
            # Fell in hole
